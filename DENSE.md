...
| `NEXTPNR_REPLICATE=FILE` | the driver of a high-fanout net copied once per group of its sinks, each group's pins moved to their own copy (`<net>$dense_rep<k>`), as Vivado's fanout optimisation replicates drivers. FILE is `dense/replicate_groups.py`'s: the net's sink pins clustered by where a placement put them. The pass runs after packing; a file naming more than 0.1% pins the net lacks is refused | only when set |
